...
internal static class EndpointRegistrationGenerator
{
    public static string GenerateEndpointRegistrationCode(List<EndpointMethod> endpointMethods)
    {
        ...
        sb.AppendLine("using Microsoft.AspNetCore.Http;"); // For Accepts/Produces extensions and HttpContext

        var namespaces = endpointMethods.Select(m => m.ClassNamespace).Distinct().Where(n => !string.IsNullOrEmpty(n));
        foreach (var ns in namespaces)
        {
            sb.AppendLine($"#pragma warning disable CS0169 // The field is never used");
